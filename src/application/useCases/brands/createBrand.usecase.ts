...
import { CreateBrandRequestDTO, CreateBrandResponseDTO } from "../../dto/brands/createBrand.dto";
...
export class CreateBrandUseCase implements ICreateBrandUseCase {
  constructor(
    ...
    async execute(req: CreateBrandRequestDTO): Promise<CreateBrandResponseDTO> {
        ...
        const partnerBrands = await this.brandRepository.findByPartnerId(req.partnerId);
        const duplicateBrand = partnerBrands.find(brand => brand.getName().toLowerCase() === req.name.toLowerCase());
        ...
        const newBrand = new Brand(newUniqueId, req.name, partnerId, true);
