...
} from "../../dto/products/createProduct.dto";
...
export class CreateProductUseCase implements ICreateProductUseCase {
  ...
  async execute(req: CreateProductRequestDTO): Promise<CreateProductResponseDTO> {
    ...
    const brandProducts = await this.productRepository.findByBrandId(
      req.brandId,
    );
    const duplicatedProduct = brandProducts.find(
      (product) => product.getName().toLowerCase() === req.name.toLowerCase(),
    );
    if (duplicatedProduct)
      throw new BadRequestError("Product already exists for this brand");

    const newUniqueId = new UniqueId(req.id);
    const brandId = new UniqueId(req.brandId);
    const newProduct = new Product(newUniqueId, req.name, brandId, req.active);
